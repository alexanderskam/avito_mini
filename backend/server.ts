...
await fastify.register(cors, {
    origin: 'http://localhost:5173', // или true для всех
    methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
});

// Мидлвар для искусственной задержки, но пропускаем OPTIONS
await fastify.register((await import('@fastify/middie')).default);
fastify.use((req, res, next) => {
    if (req.method === 'OPTIONS') return next(); // пропускаем preflight
...
interface ItemUpdateRequest extends Fastify.RequestGenericInterface {
    ...
    Body: z.infer<typeof ItemUpdateInSchema>;
}
...
fastify.put<ItemUpdateRequest>('/items/:id', (request, reply) => {
    ...
    try {
        ...
        const newCategory =
            (request.body as any).category || existingItem.category;

        let newParams = request.body?.params;
